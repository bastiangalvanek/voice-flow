"""Generiert voice-flow.ico fuer Windows-Shortcuts.

Logik:
- Wenn logo.png im voice-flow/ Verzeichnis liegt → wird als Icon-Quelle benutzt
- Sonst → faellt zurueck auf gemaltes Mikrofon-Icon

Re-run nach Logo-Aenderung: `python make_icon.py`
"""
...
def make_microphone_fallback(size: int = 256) -> Image.Image:
    """Rotes Mikrofon-Icon als Fallback wenn logo.png nicht da ist."""
...
def _square_pad(img: Image.Image) -> Image.Image:
    """Pads non-square image to square with transparent border."""
    w, h = img.size
    if w == h:
        return img
    side = max(w, h)
...
def load_logo_or_fallback() -> tuple[Image.Image, str]:
    if LOGO_PNG.exists():
        ...
        # Auf exakt 256 padden wenn thumbnail kleiner zurueckkommt
        if img.size != (256, 256):
    ...
    return make_microphone_fallback(256), "Mikrofon-Fallback (kein logo.png)"
